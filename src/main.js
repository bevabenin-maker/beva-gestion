...
function receiptHtml(payment) {
  ...
  const shareText = [
    `BEVA — ${cancelled ? 'Annulation de versement' : 'Reçu de paiement'}`,
    `N° reçu : ${receiptNumber(payment)}`,
    `Date : ${new Date(payment.paid_at).toLocaleDateString('fr-FR')}`,
    `Élève : ${payerName}`,
    `Dossier : ${dossier}`,
    `Formation : ${formation?.name || 'Non précisée'}`,
    `Versement : ${money(payment.amount)}`,
    `Moyen : ${paymentMethodLabel(payment.method)}`,
    cancelled ? `Statut : Annulé — ${payment.cancellation_reason || 'motif non précisé'}` : pending ? 'Statut : Paiement en attente d’affectation' : `Reste à payer : ${money(remaining)}`,
    'BEVA — Bénin Vivi Académie'
  ].join('\n')
  ...
  </style></head><body><div class="toolbar"><button onclick="window.print()">Télécharger / imprimer le PDF</button><button class="share" onclick="shareReceipt()">Partager vers WhatsApp</button><button class="copy" onclick="copyReceipt()">Copier le reçu</button></div><main class="receipt"><section class="top"><img class="logo" src="${receiptAsset('beva-logo.png')}" alt="BEVA"><div class="receipt-title"><h1>${cancelled ? 'ANNULATION DE VERSEMENT' : 'REÇU DE PAIEMENT'}</h1><p>Date : <b>${new Date(payment.paid_at).toLocaleDateString('fr-FR')}</b></p><div class="receipt-number">N° ${esc(receiptNumber(payment))}</div></div></section><div class="bar">INFORMATIONS DE L'ÉTUDIANT(E)</div><section class="info"><div class="field"><b>Nom et prénom</b><strong>${esc(payerName)}</strong></div><div class="field"><b>N° de dossier</b><strong>${esc(dossier)}</strong></div><div class="field"><b>Formation</b><strong>${esc(formation?.name || 'Non précisée')}</strong></div><div class="field"><b>Statut tarifaire</b><strong>${status}</strong></div></section><section class="payment-box"><h2>DÉTAIL DU VERSEMENT</h2><div class="payment-row payment-head"><b>Description</b><b>Montant reçu</b><b>Mode</b><b>Référence</b></div><div class="payment-row payment-detail"><strong>${esc(pending ? 'Paiement en attente d’affectation' : paymentObject)}</strong><strong>${money(payment.amount)}</strong><span>${esc(paymentMethodLabel(payment.method))}</span><span>${esc(payment.reference || '—')}</span></div></section><div class="amount"><span>MONTANT DU VERSEMENT (FCFA)</span><b>${money(payment.amount)}</b></div>${totals}<p class="notice">${cancelled ? 'Ce document confirme l’annulation du versement indiqué.' : 'Ce reçu tient lieu de preuve de paiement. Conservez-le précieusement.'} - BEVA, Abomey-Calavi, Bénin</p><div class="stamp-wrap"><img class="stamp" src="${receiptAsset('beva-stamp.png')}" alt="Cachet BEVA"><div class="signature">La Direction</div></div><p class="contact">BEVA - Bénin Vivi Académie - Abomey-Calavi, Bénin</p><script>const receiptShareText=${JSON.stringify(shareText)};async function copyReceipt(){try{if(navigator.clipboard&&window.isSecureContext){await navigator.clipboard.writeText(receiptShareText)}else{const t=document.createElement('textarea');t.value=receiptShareText;document.body.append(t);t.select();document.execCommand('copy');t.remove()}alert('Reçu copié. Ouvrez WhatsApp puis collez le message.') }catch(e){alert('Copie impossible. Utilisez le bouton Partager vers WhatsApp.')}}async function shareReceipt(){if(navigator.share){try{await navigator.share({title:document.title,text:receiptShareText});return}catch(e){if(e&&e.name==='AbortError')return}}window.open('https://wa.me/?text='+encodeURIComponent(receiptShareText),'_blank')}</script></main></body></html>`
}
