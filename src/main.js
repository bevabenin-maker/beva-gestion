...
function receiptHtml(payment) {
  ...
  const formationName = enrollment
    ? formationNameFor(enrollment)
    : (state.formations.find(x => x.id === payment.pending_formation_id)?.name || 'Non précisée')
...
function loginView() {
  ...
  document.querySelector('#login-form').addEventListener('submit', async event => {
    ...
    if (error) {
      // Supabase deliberately keeps credential failures generic.  Keep the
      // technical detail in the browser console for diagnosis, but do not
      // leak account information on this staff-only login page.
      console.error('Erreur de connexion BEVA', error)
    ...
    <div class="shell">
      ...
      <main class="main">
        ...
        <section id="dashboard" class="section">
          <div class="cards">
            <div class="card"><div class="label">Étudiants enregistrés</div><div class="value">${scopedStudents().length}</div></div>
...
function studentMobileCards(students) {
  ...
  return students.map(student => {
    const items = confirmedEnrollmentsFor(student.id)
    const formations = items.map(formationFor).filter(Boolean).map(item => item.name)
    const age = studentAge(student)
    const statusClass = student.status === 'actif' ? 'ok' : student.status === 'abandonne' ? 'due' : 'warning'
    return `<article class="student-mobile-card"><div><span class="code">N° ${esc(student.intake_student_number || '—')}</span><span class="badge ${statusClass}">${esc(studentStatusLabel(student.status))}</span></div><button class="link-btn student-payment-history" data-id="${student.id}" title="Voir son historique de paiements"><strong>${esc(student.last_name)} ${esc(student.first_name)}</strong></button><p class="student-formations">${formations.length ? esc(formations.join(' · ')) : 'Aucune formation confirmée — 4 disponibles'}</p><dl><div><dt>Âge</dt><dd>${age === null ? 'Non renseigné' : `${age} ans`}</dd></div><div><dt>Confirmées</dt><dd>${items.length} / 4</dd></div><div><dt>Téléphone</dt><dd>${esc(student.phone || '—')}</dd></div></dl></article>`
  }).join('')
}
...
function studentRows(students) {
  return students.map(student => {
    ...
    return `<tr>
      <td class="code">N° ${esc(student.intake_student_number || '—')}</td>
      ...
      <td><span class="badge ${student.status === 'actif' ? 'ok' : student.status === 'abandonne' ? 'due' : 'warning'}">${esc(studentStatusLabel(student.status))}</span> <span class="badge ${items.length ? 'ok' : ''}">${items.length ? `${items.length} confirmée${items.length > 1 ? 's' : ''}` : '4 disponibles'}</span></td>
...
function paymentPanel() {
  ...
  const enrollmentRows = displayed.map(enrollment => {
    ...
    return `<tr><td><strong>${student ? `${esc(student.last_name)} ${esc(student.first_name)}` : '—'}</strong></td>
  ...
  return `${pendingPaymentPanel()}<div class="panel financial-panel"><div class="panel-head"><div><h2>Suivi financier par formation</h2><p class="muted">Le filtre mensuel ne montre que les dossiers actifs. Les dossiers abandonnés ou non actifs restent visibles dans l’historique.</p>${monthlySummary}</div><label class="payment-filter">Échéance<select id="payment-month-filter"><option value="all" ${state.paymentMonth === 'all' ? 'selected' : ''}>Vue complète</option>${monthOptions}<option value="settled" ${state.paymentMonth === 'settled' ? 'selected' : ''}>Soldés — 3 mois</option></select></label></div><div class="mobile-payment-list">${mobileCards || '<div class="empty">Aucun dossier pour cette échéance.</div>'}</div><div class="table-wrap desktop-table">
    ...
    <table><thead><tr><th>Date</th><th>Étudiant</th><th>Formation</th><th>Montant</th><th>Reste à payer</th><th>Mois / tranche</th><th>Moyen</th><th>Référence</th><th></th></tr></thead>
...
function applicationErrorView(error) {
  ...
  app.innerHTML = `
    <div class="login-page">
      <section class="login-brand"><div class="brand-mark">BEVA</div><h1>Accès confirmé.<br>Affichage à réparer.</h1><p>Votre compte est bien connecté. Les données et les paiements n’ont pas été modifiés.</p></section>
      <section class="login-panel"><div class="login-card">
        <h2>Une erreur d’affichage est survenue</h2>
        <p class="muted">Réessayez d’abord d’ouvrir votre espace. Si le problème persiste, le message ci-dessous permettra de le corriger sans vous déconnecter.</p>
        <p class="error">${esc(detail)}</p>
...
async function startFromAuthenticatedSession() {
  try {
    ...
    // A rendering/data issue must not invalidate an otherwise valid staff
    // session. Keeping it open makes the problem recoverable and diagnosable.
...
async function start(session) {
  ...
  await startFromAuthenticatedSession()
}
